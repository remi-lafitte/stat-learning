...
```{r fig.align='center'}
p1<-ggplot(d_cue, aes(x = genre, y = mean, col = genre)) +
    geom_errorbar(aes(ymin = d_cue$mean - d_cue$sd, ymax = d_cue$mean + d_cue$sd), width = 0.2, size = 2, 
                  col ="red", alpha = 0.4)+
  geom_jitter(data = d, aes(genre, y = tr, col = genre), size = 3, alpha = 0.4, width = 0.05)+
  geom_point(size = 5)+
...
f<-rnorm2(n = 12, mean = 532, sd = 20) 
h<-rnorm2(n = 12, mean = 508, sd = 20) 
...
d_cue2<-
  ...
  mutate(
    mean_tr = mean(new_tr),
    diff_carre = (new_tr - mean_tr)^2
  ) %>% 
  summarize(sum = sum(diff_carre), mean = mean(mean_tr)) %>% 
  mutate(n = 12) %>% 
  mutate(sd = sqrt((sum / n)), sd = sqrt((sum / n)))


p2<-ggplot(d_cue2, aes(x = genre, y = mean, col = genre)) +
    geom_errorbar(aes(ymin = d_cue2$mean - d_cue2$sd, ymax = d_cue2$mean + d_cue2$sd), width = 0.2, size = 2, 
                  col ="red", alpha = 0.4)+
  geom_jitter(data = d2, aes(genre, y = new_tr, col = genre), size = 3, alpha = 0.4, width = 0.05)+
  geom_point(size = 5)+
  scale_y_continuous(limits=c(300, 700))+
  labs(title = "SD = 20 ms")
...
**A noter que l'écart type de $H0$ (*erreur type*) est déterminé sur la base de l'écart type de l'échantillon. A partir de cette distribution $H0$ on va pouvoir situer à quel point notre $t$ observé se trouve loin de 0.** 
