...
```{r}
h<-as_tibble(rnorm(mean = 521, sd = 5, n = 200))
f<-as_tibble(rnorm(mean = 519, sd = 5, n = 200))
m<-cbind(h,f)
colnames(m)<-c("Allemagne", "Espagne")
pop<-m %>% gather(pays, tr)
data<-pop
p<-ggplot(data, aes(x = pays, y =tr, col = pays, fill = pays))+
    geom_jitter(size = 3, alpha = 0.7, width = 0.05)+
  stat_summary(aes(group = 1),fun = "mean", geom = "line", col = "black")+
  stat_summary(fun = mean,
               geom = "errorbar",
  ...
  guides(col = F, fill =F)+
  theme_bw(base_size = 14)
png(filename = "sondage.png")
p
dev.off()
write.table(file = "sondage.txt",capture.output(t.test(h,f)), quote = F,
            row.names = F)
t.test(h,f)
x<-c(1,3)
sd(x)
